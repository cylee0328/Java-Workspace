...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println(hs1); // 중복저장 x , 저장 순서유지 x
		...
		// 저장 순서 유지 x, 중복 저장 o -> 동일객체로 판단되지 않이 때문.
		// 값이 추가될때 equals(), hashCode()로 비교 후 둘 다 결과가 true면 동일객체로 판단.
	    
		// HashSet 에 객체를 담을 때 내부적으로 equals()로 비교 + hashCod()값이 일치하는지도 비교
		...
		// Student클래스에 equals() 오버라이딩 : 새 필드 값(name, age, score)이 일치하면 true, 아니면 false
		...
		while(it.hasNext()) {// it에 next로 뺴올 값이 있는가? 라고 물어보는 메소드
			it.next();
