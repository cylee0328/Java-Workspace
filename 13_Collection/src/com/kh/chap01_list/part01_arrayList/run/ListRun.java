...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 *  == 컬렉션
		 *  => 다량의 데이터들을 관리하고자 할 때 배열 가지고 충분히 사용했엇음, 단 배열의 단점들을 보완한 것이 컬렉션.
		 *  
		 *  배열과 컬렉션의 차이점
		 *  - 배열의 단점
		 ...
		 *  컬렉션의 3가지 분류
		 *  
		 *  List계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서유지됨(index의개념이 있음)/ 중복값 허용.
		 *           ex) ArrayList, Vector, LinkedList => ArrayList가 제일많이 쓰임(90%이상)
		 *  Set 계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 x(index개념이 없음) / 중복값 허용 x
		 *           ex) HashSet, TreeSet
		 ...
		 * 해당 컬렉션에 다양한 타입의 데이터값들이 담길수 있음.(Defualt E == Ojbect)
		 * 
		...
		//7. subList(int index1, int index2) : 해당 리시트로 부터 인덱스1에서 인덱스2까지의 데이터 값들
		//									   추출해서 새로운 List로 반환시켜주는 메소드
		
		List<Music> sub = list.subList(0, 1); // 0<= 인덱스 범위 <2
