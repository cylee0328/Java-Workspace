...
public class Run {
	...
	public static void main(String[] args) {
		...
		/*
		 * 계층구조를 보면 List나 Set 계열은 Collection을 구현한 클래스 -> 데이터를 추가할 때 add메소드
		...
		// 1. 저장순서 유지x
		// 2. value값이 동일하다고해도, key 값이 중복되지 않기 때문에 잘 저장됨.
		
		hm.put("포카칩", new Snack("매운맛", 1200));
		// 중복된 key값의 value 값으로 덮어씌우기됨.
		System.out.println(hm);
		
		// 2. get(Object key) : V => 컬렉션에서 해당 키 값의 value값을 돌려주는 메소드
		System.out.println(hm.get("다이제"));
		
		Snack s = hm.get("칸쵸"); // 제너릭설정을 해놨기 때문에 Snack타입 객체로 강제형변환 하는 과정이 생략되었다.
		...
		// iterator()는 List와 Set계열에서만 쓰는 메소드이기 때문에 바로 사용불가.
