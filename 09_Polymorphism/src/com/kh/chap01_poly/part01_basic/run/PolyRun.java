...
public class PolyRun {
	...
	public static void main(String[] args) {
		...
		Chil1 c1 = new Chil1(); // 기본생성자에 슈퍼생성잘르 호출하는 구문이 항상 있음. 따라서 부모객체사용가능.
		c1.printChild1();
		c1.printParent();
		//c1 레퍼런스로 Parent(부모), child1(자식) 에있는 필드나 매서드 둘다 접근 가능.
		//c1 레퍼런스변수가 Parent에 있는 메소드에 접근 시 자동으로 형변환된채 진행됨
		//즉 자식에서 부모클래스로 자동형변환(업캐스팅) -> 작은 것에서 큰 것으로 형변환 된 것과 같은 이치.
		
		//3. 부모타입 레퍼런스 변수로 자식 객체를 다루는 경우(다형성이 적용된 개념)
		System.out.println("3. 부모 타입 레퍼런스 변수로 자식 객체를 다루는 경우(다형성 적용된 개념)");
		Parent p2 = /*(Parent)*/ new Chil1();
		/*
		 * 양쪽에 자료형이 다른데도 불구하고 오류 발생x
		 * Parent형으로 '자동형변횐'이 되고 있기 때문.
		 * 상속 구조에서는 클래스 형변환이 가능하다.
		 */
		p2.printParent(); //p2라는 레퍼런스 변수로는 Parent에만 접근이 가능.
		((Chil1) p2).printChild1(); //단, 자식객체로 접근하고싶을 때 Child1형으로 강제형변환후 Child1에 접근 가능.
		...
		// 단, 다형성이 적용되면 부모타입래퍼런스 변수로 모든 자식객체를 다 받아줄 수 있다.
		System.out.println("======= 다형성 접목한 객체배열 =======");
		
		Parent[] arr = new Parent[4];
		arr[0] = new Chil1(1,2,3);
		arr[1] = new Child2(4,5,6);
		arr[2] = new Chil1(5,5,5);
		arr[3] = new Child2(1,5,65);
		// 하나의 부모타입으로 모득 자식객체들을 받을 수 있음 => 편리, 코드 수 감소, 생산성 증대
		
		((Chil1)arr[0]).printChild1();
		((Child2)arr[1]).printChild2();
		((Chil1)arr[2]).printChild1();
...
//				((Chil1)arr[i]).printChild1();
//			} else {
//				((Child2)arr[i]).printChild2();
//			}
			
			// 단, 오버라이딩을 이용면 굳이 형변환 안해도됨.
			arr[i].print();
			/*
			 *  * 동적바인딩 : 프로그램이 실행되기전에 자동으로 컴파일되면 정적으로 현재 레퍼런스 변수의 메소드를 가리키게됨.
			 *  단, 참조하고 이는 자식클래스에 해당 메소드가 오버라이딩 되어 있다면
			 *  프로그램을 시작할 때는 동적으로 그 자식클래스의 오버라이딩된 메소드를 찾아서 실행하게됨.
