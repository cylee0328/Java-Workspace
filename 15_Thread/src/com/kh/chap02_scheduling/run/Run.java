...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 우선순위 변경 전
		// 스레드는 기본적으로 1~10의 우선순위로 5의 우선순위를 가지고 있다.
		// 메모리가 모자랄 때 우선순위를 정할 수 있음
		// 스레드의 우선순위를 정하지 않으면 OS(운영개체)가 임의로 수행시킨다.
		System.out.println("==============우선순위 변경 전================");
		System.out.println(Thread.currentThread().getName()+"의 우선순위"+Thread.currentThread().getPriority());
		System.out.println(t1.getName()+"의 우선선위 : " + t1.getPriority());
		System.out.println(t2.getName()+"의 우선선위 : " + t2.getPriority());
		System.out.println(t3.getName()+"의 우선선위 : " + t3.getPriority());
		...
		/*
		 * 메인스레드가 종료되더라고 실행 중인 스레드가 하나라도 있다면 프로세스는 종료되지 않는다.
		 * 해결책 : start()시키기전에 우선순위, 주종관계를 설정해야함.
		 * 
		 * 주종관계 설정
		 * main 스레드가 종료시 다른스레드도 종료시키기 위해서 반드시 setDaemon사용.
		 * 주의점은 스레드 중 한개라도 setDaemon으로 설정시 모든 스레드에 setDaemon설정을 해줘야함.
		 * 
		 */
		System.out.println("메인 스레드의 끝부분");
		// 시작순서는 우선순위와 다를 수 있찌만 종료순서는 우선순위(priority)가 높은 순으로 끝나게 됨.(항상 그렇지않음)
