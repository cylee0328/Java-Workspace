 ...
 * 
 * 2. 지역변수
 * 		생성시점 : 특정구역({}) 실행(호출)시 메모리 영역에 할당(ex 메소드 실행 시)
...
public class FieldTest1 {// FieldTest1 클래스 시작
	...
	public void test(int num) { // test메소드 시작
		...
		/*
		 * 멤버변수 출력 시 에러가 안나는 이유
		 * 해당 클래스 객체 생성 시 jvm이 값을 해당 필드로 자동으로 대입(초기화)시켜 줬기 때문
		 * 물론, 명시적으로 초기화도 가능함.
		 */
		System.out.println(global);
		
		// 매개변수 출력 시 에러 안나는 이유
		// 해당 메서드(test)호출 시 반드기 값을 넘겨줘야 호출이 가능하기 때문에 num에는 반드시 어떤 값이든 초기화가 될 것.
