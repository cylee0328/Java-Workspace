...
public class A_Array {
	...
	public void method2() {
		...
		/*
		 * 우선 대입연산기준 왼쪽이 먼저 실행되므로 arr이라는 int형배열 박스가 먼저 만들어짐.(stack영역에)
		 * 그 다음 int[5]짜리 방이 heap 영역에 생성됨.(new 키워드가 붙은 녀석은 항상 heap영역에 값이 할당됨)
		 * 5칸짜리 방이 만들어진 후에 각 방에 기본값으로(int 0으로)데이터가 초기화되고, 주소값이 할당됨 (0x123)
	...
	public void method3() {
		 ...
		 * -> heap이라는 영역에는 절대로 빈 공간이 존재할 수 없기 때문.
...
//		}
		...
		System.out.println("arr의 해시코드 값 : " + arr.hashCode());
		/*
		 * 항상 고유한 주소값 부여, 기존에 생성되었던 주소와 절때로 겹치지 않음.
		 * 기존에 참조하고 있던 연결이 끊기고 새로운 곳 참조
		 * 
		 * 연결이 끊어진 기존의 배열은 heap영역에 떠다님
		 * 일정 시간이 지나서도, 사용이 되지 않으면 "가비지 컬렉터(GC)"가 삭제시켜줌.
		 * (자동 메모리 관리)
		 *  => 자바에서 자동메모리 특징. 개발자가 코드에만 신경쓸 수 있게 해줌.
		 */
	...
	public void method7() {
		...
		// 배열 선언 및 동시에 초기화(대입)까지 한번에 끝내는 방법
		// 방법1.
	...
	public void method8() {
		...
		//3. 반복문 활용해서 0~ 마지막인덱스까지 배열에 담긴값 출력.
		// 출력예시) arr[x] : xx
	...
	public void method9() {
		...
		sc.nextLine();
		//2. 반복문 활용. 매번 사용자에게 과일명 입력받아 그 값을 매 인덱스자리에 대입
		for(int i=0; i<arr.length; i++) {
	...
	public void method10() {
		...
		//2. char 배열 생성해두기 (배열크기 ==문자열의 길이만큼)
	...
	public void method11() {
		// 1. 사용자에게 배열의 길이를 입력 받은 후, 해당크기만큼 정수배열 생성
		// 2. 반복문을 활용해서 0~마지막 인덱스까지 순차적으로 접근하면서 값 대입
		//    -> 랜덤값(1~100)
		// 3. 반복문을 활용해서 0~마지막인덱스까지 배열에 담긴값 출력과 동시에.
	...
	public void method12() {
		...
		double sum = 0;
		for(int i=0; i<height.length; i++) {
			System.out.printf("height[%d] : %.1f \n", i, height[i]);
			sum+=height[i];
		}
		System.out.printf("키 총합 : %.1fcm, 평균 키 : %.1fcm", sum, sum/3);
	}
}
