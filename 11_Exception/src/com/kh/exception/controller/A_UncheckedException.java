...
public class A_UncheckedException {
	 ...
	 * - NullpointerException : 레퍼런스가 아직 null임에도 불구하고 접근하려고할 때 발생하는 예외
	...
	public void method1() {
		...
		try {
			...
			System.out.println("0으로 나눌 수 없습니다.");
			e.printStackTrace(); // 현재발생한 오류를 추적할 수있는 로그정보제공
		}
		System.out.println("프로그램 종료.");
 	}
	...
	public void method2 () {
		...
		try {
			int num = sc.nextInt();// InputmissmatchException : 정수 이외의 값을 입력시 발생함.
			System.out.println("나눗셈 연산 결과 : " + (10/num)); // 예외발생할 수 있음
	...
	public void method3() {
		//ArrayIndexOutOfException : 배열의 부적절한 인덱스로 접근시 발생하는 예외
		// NegativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
...
//			int[] arr = new int[Math.abs(size)]; // 방법2. 항상 양의정수값이 들어갈 수 있도록 abs메서드 사용
//		}
//		
//		try {
...
//			System.out.println("예외가 발생함. 배열의크기를 음수로 입력했거나, 부적절한 인덱스로 접근한 것 같습니다.");
//		} // 다중 catch블럭이 줄어들었음. 정확히 어떤 에러인지 알 수 없음.
	 ...
	 * - 예외처리 구문으로도 해결 가능 => 예와가 발생했을때를 대비해서 그 때 실행할 내용을 정의해 두는 것.
	 * 
